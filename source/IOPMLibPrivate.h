...
/*
 æ* For use by LoginWindow only.
 æ* facility - Pass CFSTR(kIOPMLoginWindowSecurityDebugKey) for the facility
 æ* data - Pass a pointer to a variable containing one byte of data.
 æ* dataCount - Pass the integer 1.
 æ*/
...
/*
 æ* Returns data describing the sleep failure (if any) that occured prior to the system booting.
 æ*
 æ* This routine will return the same CFDictionary over the lifetime of a given boot - it does not return
 æ* dynamic information after each sleep/wake. It only returns information pertaining to the last
 æ* failed sleep/wakeæbefore booting up.
 æ*æ
 æ* If NULL, then the last sleep/wake was successful, or we were unable to determine whether
 æ* there was a problem.
 æ* If non-NULL, Caller must release the returned dictionary.
 æ*
 æ* kIOPMSleepWakeFailureLoginKey points to a CFNumber containing LW's 8-bit code.
 æ* kIOPMSleepWakeFailureUUIDKey points to a CFStringRef containing the UUID associated with the failed sleep.
 æ* kIOPMSleepWakeFailureDateKey points to the CFDate that the failed sleep was initiated.
 æ*/
